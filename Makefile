.PHONY: build network containers remove clean
...
containers: build network
	...
	docker run --privileged --rm -ti -d \
		--name mydb-example --hostname mydb-example --ip 10.0.2.254 --network srv \
		midebian-mydb-example
...
build:
	...
	docker build --rm -f src/Dockerfile --tag midebian-mydb-example src/
